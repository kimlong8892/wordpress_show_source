<?php
if (!function_exists('ksa_upload_from_url')) {
    function ksa_upload_from_url($image_url, $attach_to_post = 0, $add_to_media = true, $post_id = null, $set_thumb = false) {
        @$remote_image = fopen($image_url, 'r') ?? null;
        ...
        $filename = $filename_no_ext . rand(100000, 99999999) . '-' . time() . "." . $extension;
...
if (!function_exists('getImageByPostId')) {
    function getImageByPostId($postId) {
        $imageUrl = wp_get_attachment_url( get_post_thumbnail_id($postId));

        return str_replace(get_site_url(), getCurrentBaseUrl(), $imageUrl);
    }
}
